...
public:
    ...
    cv::Mat infer(cv::Mat &image) {
        ...
        // 3.2 创建输入值
        Ort::Value input_tensor = Ort::Value::CreateTensor<float>(memory_info, resized_image.ptr<float>(),
                                                                  resized_image.total(), input_dims[0].data(),
                                                                  input_dims[0].size());
        // 3.3 推理 只传递输入
        vector<Ort::Value> output_tensors;
        try {
            // output_tensors[0]: [1 2 256 256] 需要 [1, 2, :, :]
            output_tensors = session.Run(this->runOptions, input_node_names.data(), &input_tensor, input_nums,
                                         output_node_names.data(), output_nums);
        }
        catch (Ort::Exception &e) {
            cout << e.what() << endl;
        }

        // 4.将热力图转换为Mat TODO: [1, 2, :, :] 现在是全部放进去的,缺少softmax
        auto *output0 = output_tensors[0].GetTensorMutableData<float>();
//        int image_total = this->meta.infer_size[0] * this->meta.infer_size[1];
//        vector<float> v_output0_1;
//        float output0_1[65536]; // 256 * 256
//        for (int i = 0; i < image_total; i++) {
//            v_output0_1.push_back(output0[image_total + i]);
//            output0_1[i] = output0[image_total + i];
//        }
//        cout << image_total << endl;
//        cout << v_output0_1.size() << endl;
//        cv::Mat temp = cv::Mat(v_output0_1);
//        temp = temp.reshape(0, vector<int>{256, 256});
        cv::Mat anomaly_map = cv::Mat(cv::Size(this->meta.infer_size[1], this->meta.infer_size[0]),
                                      CV_32FC1, output0);

        // 5.后处理:缩放到原图
        cv::Mat result = Resize(anomaly_map, meta.image_size[0], meta.image_size[1], "bilinear");
