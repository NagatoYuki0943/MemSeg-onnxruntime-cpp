...
void single(const wchar_t* model_path, string& meta_path, string& image_path, string& save_dir,
    ...
    // 3.推理单张图片
    cv::Mat anomaly_map = inference.infer(image);

    // 4.生成其他图片(mask,mask边缘,热力图和原图的叠加)
    vector<cv::Mat> images = gen_images(image, anomaly_map);
    // time
    ...
    saveImages(images, image_path, save_dir);
...
int main() {
    const wchar_t* model_path = L"D:/ai/code/abnormal/MemSeg-TooTouch/saved_model/mvtec/MemSeg-bottle/best_model.onnx";
    string param_path         = "D:/ai/code/abnormal/MemSeg-TooTouch/saved_model/mvtec/MemSeg-bottle/config.json";
    string image_path = "D:/ai/code/abnormal/MemSeg-TooTouch/datasets/MVTec/bottle/test/broken_large/000.png";
    string image_dir  = "D:/ai/code/abnormal/MemSeg-TooTouch/datasets/MVTec/bottle/test/broken_large";
    string save_dir = "D:/ai/code/abnormal/MemSeg-onnxruntime-cpp/cmake/result"; // 注意目录不会自动创建,要手动创建才会保存
    string device = "cuda";
